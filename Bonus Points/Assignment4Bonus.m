...
end_character = "§";
end_character_index = char_to_ind(end_character);
...
x0 = zeros(Dim.K, 1);
x0(1) = 1;
Y = synthesizeText(RNN, Dim, h0, x0, n, end_character_index);

string = oneHotToString(Y, ind_to_char);
length(string);


% === Running gradient descent ===
max_char = 1e6;
RNN_trained = MiniBatchGD(book_data, RNN, Dim, char_to_ind, ind_to_char, seq_length, n_epochs, end_character_index, max_char)


function RNN = MiniBatchGD(book_data, RNN, Dim, char_to_ind, ind_to_char, seq_length, n_epochs, end_character_index, max_char)
    % Compute helpful variables
    e_values = 1:seq_length:(length(book_data) - seq_length - 1);
    n_updates = length(e_values) * n_epochs
    j = 1;
    max_iter = floor(25 * max_char / seq_length)
    smooth_loss = zeros([min(n_updates, max_iter), 1]);
    ...
    for i = 1:n_epochs
        ...
        for e = e_values
            ...
            % Compute loss
            [loss, A, H, ~, P] = ComputeLoss(X_batch, Y_batch, RNN, Dim, h_prev, end_character_index);
            ...
            if j == 1
                smooth_loss(j) = loss / seq_length;
            else
                smooth_loss(j) = 0.999 * smooth_loss(j - 1) + 0.001 * loss / seq_length;
            end
            
            if j == max_iter
                final_loss = smooth_loss(j)
...
%                     x = X_batch(:, 1); % Previous character
                    x = zeros(size(X_batch(:, 1))); 
                    x(end_character_index) = 1; % New tweet
                    synthesizedY = synthesizeText(RNN, Dim, h_prev, x, 140, end_character_index);
                    tweet = oneHotToString(synthesizedY, ind_to_char);
                    length_of_tweet = length(tweet)
                    tweet
                end
            end
...
function [book_data, char_to_ind, ind_to_char, K] = getContainers()
    % Read book
    book_fname = 'data/Tweets100k.txt';
...
function Y = synthesizeText(RNN, Dim, h0, x0, n, end_character_index)
    n_max = min([n, 140]); % Hard cap length of tweet
    ...
    for t = 1:n_max
        ...
        % Stopping when end-character is reached
        if x_next(end_character_index) == 1
            Y = Y(:, 1:t-1); % Save current tweet
            return
        end
...
function [loss, A, H, O, P] = ComputeLoss(X, Y, RNN, Dim, h0, end_character_index)
    n = size(X, 2);
    h_t = h0;
    ...
    for t = 1:n
        ...
        if X(end_character_index, t) == 1 % End of tweet
            h_t = zeros(size(h_t)); % Re-initalize h_prev
        end
    end
